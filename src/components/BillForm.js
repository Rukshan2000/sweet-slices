...
function BillForm({ onGenerateBill }) {
    ...
    const handleSubmit = (e) => {
        ...
        onGenerateBill({ ...formState, totalAmount, discountedTotal, totalDue });
    };
    ...
    return (
        <form className={`p-6 ${lightMode ? 'bg-white text-gray-800' : 'text-white bg-gray-800'} rounded shadow-md`} onSubmit={handleSubmit} style={{ boxShadow: '0 4px 6px -1px rgba(255, 215, 0, 0.5), 0 2px 4px -1px rgba(255, 215, 0, 0.06)' }}>
            <h2 className="mb-4 text-2xl font-bold">Bill Form</h2>
            <div className="grid gap-4 mb-4 sm:grid-cols-1 lg:grid-cols-2">
                <div>
                    <label className="block mb-2 font-bold">Bill Number</label>
                    <input type="text" name="billNumber" value={formState.billNumber} onChange={handleChange}
            ...
            <button type="submit" className="px-4 py-2 mb-4 text-white bg-green-500 rounded">Generate Bill</button>
        </form>
    );
}
